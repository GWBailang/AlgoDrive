最近用 cf 遂穿把旧电脑整成服务器了，于是写了个简单的网盘，由 AI 辅助完成，用于学习目的。当然网盘只是为了自己用。
...
* **$O(1)$ 内存分段上传**: 采用前端 `Blob.slice()` 切片并分段上传与后端 `shutil.copyfileobj` 流式合并，无论上传 10GB 还是 100GB，服务器内存占用始终保持平稳。
